...
# Branch patterns used to restrict WI extraction to main integration lines
...
def _resolve_branches(repo_root: Path) -> list[str]:
    """Return remote branch refs matching *_WI_BRANCH_PATTERNS*."""
    result = subprocess.run(
        ["git", "-C", str(repo_root), "for-each-ref", "--format=%(refname:short)"] + _WI_BRANCH_PATTERNS,
        capture_output=True,
        text=True,
        encoding="utf-8",
        errors="replace",
...
def _fetch_merge_bodies(
    ...
    """Return ``{sha: full_message_body}`` for PR merge/squash commits.

    Only considers commits reachable from ``ng/main`` and
    ``ng/release/*/main`` branches.  Matches subjects starting with
    ``Merged PR`` — covering both true merge and squash-merge commits
    generated by Azure DevOps.  Uses control-character separators
    (RS=0x1e, US=0x1f) to reliably parse multi-line bodies.
    """
    branches = _resolve_branches(repo_root)
    if not branches:
        return {}
    ...
    cmd: list[str] = [
        "git", "-C", str(repo_root),
        "log",
        "--grep=^Merged PR", "--extended-regexp",
...
def build_git_history_graph(
    ...
    for commit in commits:
        ...
        # --- Work-item IDs (only from PR merge/squash commits) ---
        if not is_merge:
            continue  # WI extraction restricted to PR merge commits
